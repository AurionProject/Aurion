...
import gov.hhs.fha.nhinc.common.nhinccommon.SamlAttributeAssertionType;
...
public class CallbackMapProperties implements CallbackProperties {
    ...
    /* (non-Javadoc)
     * @see gov.hhs.fha.nhinc.callback.openSAML.CallbackProperties#getUserOrganizationId()
     */
    @Override
    ...
    public Map<String, String> getGenericAttributes() {
    	//List<SamlAttributeAssertionType>
		Map<String, String> genericAttributes = new HashMap<String, String>();
		String property = NhincConstants.GENERIC_ATTRS_PROP;
		if (map.containsKey(property) && map.get(property) != null) {
			...
			if (value instanceof List<?>) {
				...
				if(NullChecker.isNotNullish(values)) {
					for(Object o : values) {
						if((o != null) && (o instanceof SamlAttributeAssertionType)) {
							SamlAttributeAssertionType samlAttribute = (SamlAttributeAssertionType)o;
							genericAttributes.put(samlAttribute.getName(), samlAttribute.getValue());
						}
					}
				}
			}
